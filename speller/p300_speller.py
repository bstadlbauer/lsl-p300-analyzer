...
@author: bstad
'''
from Tkinter import Tk, Toplevel, IntVar, W, EW, Text, StringVar, Radiobutton
from ttk import Label, Button, Entry, Frame
import tkFileDialog
import numpy as np
import glob
import os
...
class ConfigParams(object):
    ...
    def save_to_file(self):
        ...
        with open(self.config_file_path.get(), 'wb') as configfile:
...
class P300Window(object):
    def __init__(self, master, parent, config):
        ...
        self.image_frame.grid(row = 0, column = 0, rowspan = self.config.number_of_rows.get(), columnspan = self.config.number_of_columns.get())
    ...
    def open_images(self):
        ...
        if len(image_paths) < min_number_of_images: 
            self.parent.print_to_console("To few images in folder: " + self.images_folder_path.get())
            return
    ...
    def create_flash_sequence(self):
        ...
        distance_between_similar_elements = (sequence_length / self.config.break_duration.get()) + 1
...
class MainWindow(object):
    ...
    def print_to_console(self, text_to_print):
        if not isinstance(text_to_print, basestring):
