...
class MainWindow(mp.Process):
    ...
    def build_main_window(self):
        ...
        from tkinter import EW, HORIZONTAL, IntVar, StringVar, Text, Tk
        ...
        self.eeg_stream_button = Button(
            self.master,
            text="Refresh",
            command=lambda: self.find_streams("EEG", self.eeg_stream_combobox, self.eeg_streams_dict, self.eeg_stream),
        )
        ...
        self.marker_stream_button = Button(
            ...
            command=lambda: self.find_streams(
                "P300_Marker", self.marker_stream_combobox, self.marker_streams_dict, self.marker_stream
            ),
        )
        ...
        self.start_recording_btn = Button(self.master, text="Start recoding", command=self.start_recording)
    ...
    def find_streams(self, stream_type, widget, stream_dict, var):
        stream_dict.clear()
        timeout = 3
        self.print_to_console("Searching for " + stream_type + " streams... (timeout = " + str(timeout) + " seconds)")
    ...
    def test_if_two_streams(self):
        if self.eeg_stream.get() != "" and self.marker_stream.get() != "":
